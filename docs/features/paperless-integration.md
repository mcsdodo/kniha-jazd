...
- **Why a separate link table instead of storing documents?** -- Paperless owns its documents and their OCR. kniha-jazd stores only the trip-link row, so the database stays small and its backups stay textual. The only pre-upgrade path from the removed local receipts is [scripts/migrate_local_to_paperless.py](../../scripts/migrate_local_to_paperless.py), which writes the old rows into Paperless before the upgrade.
