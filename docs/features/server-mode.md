...
`read_only` reflects `AppState::is_read_only()`. It is armed by the startup
migration-compatibility check in [web/src/main.rs](../../src-tauri/web/src/main.rs): a
database carrying migrations this build does not know (written by a newer image) flips
the mode and prints the unknown versions. So the field is `false` in practice except after
a rollback to an older image tag — see [read-only-mode.md](./read-only-mode.md). The UI
reads the same state through the `get_app_mode` command rather than this endpoint.
