...
The three carryover helpers all live in
[commands_internal/statistics.rs](../../src-tauri/core/src/commands_internal/statistics.rs).

**Fuel Carryover (ICE/PHEV):** `get_year_start_fuel_remaining()`
...
**Battery Carryover (BEV/PHEV):** `get_year_start_battery_remaining()`
...
**Odometer Carryover:** `get_year_start_odometer()`
...
**Vehicle Type:** the `VehicleType` enum in
[models.rs](../../src-tauri/core/src/models.rs)
- `Ice` — Internal combustion engine
- `Bev` — Battery electric vehicle
- `Phev` — Plug-in hybrid electric vehicle

**Immutability Rule:** `update_vehicle_internal()` in
[commands_internal/vehicles.rs](../../src-tauri/core/src/commands_internal/vehicles.rs)
...
**Stores:**
- [stores/vehicles.ts](../../src/lib/stores/vehicles.ts) — `vehiclesStore` and `activeVehicleStore`
- [stores/year.ts](../../src/lib/stores/year.ts) — `selectedYearStore` (defaults to current year) and the `resetToCurrentYear()` helper

**Year Picker Population:** `loadYears()` in
...
**Reactive Data Loading:** [+page.svelte](../../src/routes/+page.svelte)
- A reactive statement watches `$activeVehicleStore` and `$selectedYearStore`
- Triggers `loadTrips()` when either changes

**Vehicle Change Handling:** `handleVehicleChange()` in
[+layout.svelte](../../src/routes/+layout.svelte)
...
| File | Purpose |
|------|---------|
| [commands_internal/statistics.rs](../../src-tauri/core/src/commands_internal/statistics.rs) | `build_trip_grid_data()` and the three `get_year_start_*` carryover functions |
| [commands_internal/vehicles.rs](../../src-tauri/core/src/commands_internal/vehicles.rs) | Vehicle CRUD, including the vehicle-type immutability rule |
| [commands_internal/trips.rs](../../src-tauri/core/src/commands_internal/trips.rs) | Trip CRUD |
