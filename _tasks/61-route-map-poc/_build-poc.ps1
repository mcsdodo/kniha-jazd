...
<div id="app">
  <div id="controls">
    ...
    <label>Algorithm:
      <select id="algo">
        <option value="heuristic">Insertion heuristic</option>
...
// --- Algorithm A: Insertion heuristic ---
function generateHeuristic(targetKm) {
  // Pick primary destination D where 2 * matrix[0][D] is close to target.
  const half = targetKm / 2;
  let primary = -1, bestDelta = Infinity;
  for (let i = 1; i < N; i++) {
    const d = MATRIX.distances[HOME][i];
    if (d > targetKm * 0.9) continue; // can't reach back
    const delta = Math.abs(half - d);
    if (delta < bestDelta) { bestDelta = delta; primary = i; }
  }
  if (primary === -1) {
    primary = 1;
    for (let i = 2; i < N; i++) {
      if (MATRIX.distances[HOME][i] < MATRIX.distances[HOME][primary]) primary = i;
    }
  }

  let seq = [HOME, primary, HOME];
  let cur = totalKm(seq);

  while (Math.abs(targetKm - cur) > targetKm * TOLERANCE && seq.length - 2 < MAX_STOPS) {
    let bestErr = Math.abs(targetKm - cur), bestSeq = null;
    for (let v = 1; v < N; v++) {
      if (seq.includes(v)) continue;
      for (let p = 1; p < seq.length; p++) {
        const ns = [...seq.slice(0, p), v, ...seq.slice(p)];
        const nk = totalKm(ns);
        const err = Math.abs(targetKm - nk);
        if (err < bestErr) { bestErr = err; bestSeq = ns; }
      }
    }
    if (!bestSeq) break;
    seq = bestSeq;
  ...
  return { sequence: seq, totalKm: cur };
}
...
async function generate() {
  ...
  btn.disabled = true;
  status.textContent = 'Running ' + algo + '…'; status.className = '';

  const sessions = planSessions(target);
  cachedSessions = [];
  for (const t of sessions) {
    const t0 = performance.now();
    const result = (algo === 'ga' ? generateGA : generateHeuristic)(t);
    const elapsedMs = Math.round(performance.now() - t0);
    cachedSessions.push({ target: t, algo, result, elapsedMs });
  }
