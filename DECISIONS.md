...
3. **Migration:** [2026-09-11-120000_paperless_only_invoice_columns](./src-tauri/core/migrations/2026-09-11-120000_paperless_only_invoice_columns/up.sql) adds the two columns. [2026-09-11-130000_drop_receipts](./src-tauri/core/migrations/2026-09-11-130000_drop_receipts/up.sql) then repairs links the multi-invoice backfill mislabelled as `Other` **and** drops the table, in that order, in one folder. The repair reads `receipts`, so it cannot run after the drop; keeping both statements in one migration makes the order impossible to break. The drop is destructive and runs last, after the code compiles against the new schema.
4. **No import path for the dropped rows:** receipts already assigned to a trip survive as `paperless_trip_links` rows; every other receipt row is discarded. Nothing in the app uploads them to Paperless. Export the table before upgrading if the rows are still wanted. The automatic pre-migration backup keeps a readable copy (`sqlite3` offline), but restoring it does not undo the drop -- this build re-runs the pending migrations on start.
...
**Context:** [Task 81](./_tasks/_done/81-odometer-cascade-on-save/) closes the gap ADR-045 left open. ADR-045 stopped a save from rewriting every later row of the year, but that also meant a save could leave the chain wrong right next to the row it touched: editing one row's distance left every later odometer stale, correctable only through the manual `recalculate_odometers` dry run. The fix is not to bring the old full-year rebase back. It is to move only what the edit itself changed, and to make the user approve that move first.

**Decision:** Three planners in [trips.rs](./src-tauri/core/src/commands_internal/trips.rs) compute a `CascadePlan` ([models.rs](./src-tauri/core/src/models.rs)) before anything is written, and three commands apply it: `update_trip_cascade_internal`, `create_trip_cascade_internal`, `delete_trip_cascade_internal`. Each is reachable over RPC with a `dryRun` flag (`update_trip_cascade`, `create_trip_cascade`, `delete_trip_cascade`, [dispatcher.rs](./src-tauri/core/src/server/dispatcher.rs)). The frontend always calls dry run first, shows the plan in [OdometerCascadeModal.svelte](./src/lib/components/OdometerCascadeModal.svelte), and sends `dryRun: false` only after the user confirms. `update_trip`, `create_trip`, `delete_trip` and `recalculate_odometers` are unchanged and still reachable: the task 79 correction procedure depends on `update_trip` writing exactly the row it is given.

Edit (`plan_odometer_cascade`): if the submitted km differs from the stored km, the odometer follows it, `odometer := anchor + km`. Else, if the submitted odometer differs, the km follows it, `distance_km := odometer - anchor`. If neither differs, nothing moves at all -- a save that only fixed a typo in the purpose field leaves a row the book keeps broken on purpose (task 79) exactly as it was. The km wins when both differ because it is the number with legal weight: `distance_km` is what feeds the consumption periods and the 20% margin ([BIZ-003](#biz-003-legal-margin-limit)), while the odometer is fully reconstructible as `anchor + km`. The odometer has nothing the km does not already determine, so it is the one that gives way.

Insert (`plan_insert_cascade`): the new row's odometer is `anchor + km`, and every later row shifts by that same km -- the new row's own distance.

Delete (`plan_delete_cascade`): the shift is the removed row's **span**, `odometer - anchor`, not its recorded `distance_km`. The span is what the chain actually loses when the row disappears. The two numbers agree only when the removed row was already consistent.

In all three cases, every row **after** the affected position moves by the same `delta`. A row **before** it is unreachable by construction: `trip_order`'s anchor for row K is `odometer[K-1]`, so nothing before K can depend on what K becomes.

An edit's delta is reported as two numbers, not one, because setting `odometer := anchor + km` does two things at once: it applies the km the user typed, and it silently repairs whatever span error the row already had. `delta_from_distance` is `new_km - stored_km`, what the user typed. `delta_from_repair` is the rest -- the row's pre-existing span error, negated. The case that forced the split: row 1 of 2025 in the production book carries anchor 38056.5 from 2024-12-31, stores odometer 38145 against 88 recorded km -- a span of 88.5, half a kilometre more than the row's own distance. Editing 88 to 90 does not shift the chain by 2 km; it shifts by 1.5, because the same write also absorbs the pre-existing 0.5. The modal shows both parts (`cascade-repair` in [OdometerCascadeModal.svelte](./src/lib/components/OdometerCascadeModal.svelte)) so the user is not surprised by the smaller number.

The walk stops at the year end, matching the removed `recalculateAllOdo`. Two flags report the boundary, not one. `year_end_odometer_moved` is true whenever the year's last stored odometer changes. `next_year_chain_breaks` is true only when that AND a later year actually has trips (`mark_next_year_chain_breaks`, trips.rs). Only the second flag opens the confirmation modal on its own: appending to the newest year moves its own year end on every single save and breaks nothing, and warning on that would fire on the most common action in the app. Stopping at the year end is a decision, not a gap the cascade forgot to close: a break it leaves behind stays visible where the existing span check (ADR-043) already looks for it. The next year's first row keeps its own stored odometer and distance untouched, but its anchor -- the previous year's now-shifted last odometer -- moved by `delta`, so the two disagree by exactly that amount and the span warning catches it.

A re-dated row -- `start_datetime` changed -- cascades nothing. Changing the date moves the row inside `trip_order`, so two positions would shift, the row's old neighbour and its new one, and that is not modelled. `update_trip_cascade_internal` detects this case and writes the row alone.

A negative resulting `distance_km` is rejected with an error (task 9b; not in the original brief). `distance_km` is what `calculate_closed_period_totals` sums into a consumption period, and the period rate feeds the 20% legal margin ([BIZ-003](#biz-003-legal-margin-limit)), so a negative value would silently corrupt the legal calculation. It is silent specifically because on the odometer-wins branch the row's span is defined to equal its derived km, so the span warning (ADR-043) can never fire on it. The guard fires only on a value the caller is introducing: `plan_odometer_cascade` places its check after the no-op early return, and the re-dated branch of `update_trip_cascade_internal` compares the submitted distance against the stored row. So a row that already stores a negative distance still saves untouched when the user edits something unrelated. That keeps task 79's own finding true: a row below its anchor is a record the book keeps on purpose. Zero stays allowed; only a strictly negative value is rejected.

The frontend keeps two flags, `odoFollowsKm` and `manualOdoEdit` ([TripRow.svelte](./src/lib/components/TripRow.svelte)), but only as guards on `applyPreviewOdometer`. They decide nothing that reaches the backend. Without them, the live preview would overwrite the odometer of a row the user only opened -- a later save would then submit a changed odometer for a row nobody edited, and cascade the whole year on that basis. This is the same symptom reached twice by two different mechanisms. `d101c08` fixed the first occurrence, through a save-time clamp: fixing a typo in the purpose of a 352 km leg saved it as 1 km. That clamp is gone; task 8 removed it, and `TripRow.svelte` has no `handleOdoBlur` any more. `bd102ec`, in this same branch, is where task 81 reintroduced the symptom through the live preview instead, and restored the two guards to stop it there. Keeping the guards is what stops a third occurrence.

The apply plans again; it never replays the dry run. Each command is called twice: once with `dryRun: true` to fill the modal, once with `dryRun: false` to write. The second call reads the stored book again and computes a new plan from it. It does not write the numbers the modal showed. This is deliberate. The book can move between the two calls, and the write must follow the book, not a screen. The modal is therefore a statement about the book at the moment it opened. C1 below is what happens when the two disagree.

The whole cascade commits in one transaction. `apply_odometer_shifts` ([db.rs](./src-tauri/core/src/db.rs)) moves every shifted row inside the same transaction as the row the user edited. A shift that names a row which is no longer there returns `NotFound` and rolls the transaction back. It is never a silent no-op: a missing row means the caller planned against a book that has since moved. A cascade is one correction to a legal record, so it commits whole or not at all (task 81, spec R5).

A dry run runs in read-only mode. The three commands call `check_read_only!` only when `dry_run` is false ([trips.rs](./src-tauri/core/src/commands_internal/trips.rs)). The guard sits after the flag test on purpose, so a user whose database was written by a newer image can still see what a change would do. "Writes nothing" does not imply "works in read-only mode". That is a decision, not a consequence.

Two rules keep an open editor honest, and the UI owes both. TripGrid keys its rows by trip id, so one `TripRow` instance serves the display and every edit of that row for the life of the page. Rule 1: the row re-seeds its form from the `trip` prop only while it is not editing, or a cascade would overwrite what the user is typing. Rule 2: while a row is open, no control that can start a cascade is actionable -- copy, insert above, delete, new record, and opening a second editor. The rule is symmetric: an unsaved new row also blocks a row editor from opening, because saving that new row can insert a trip dated before the open row. Both rules are requirements on the UI, not implementation details of Svelte. Rule 1 without rule 2 is unsafe, and that gap was C1.

Rule 2 has a direction the UI does not enforce in `TripRow`, and does not need to. `handleEdit` runs only from a double-click on a display row, and the cascade modal's overlay is `position: fixed` over the whole viewport with a click-to-cancel handler ([OdometerCascadeModal.svelte](./src/lib/components/OdometerCascadeModal.svelte)). So the first click of any double-click aimed at the grid cancels the cascade, and `cascadePending` is already false by the time the second click reaches the row. Measured: the modal closes, the delete does not run, and the row opens on its stored odometer. So the `cascadePending` term in `handleEdit` never fires once the modal is up; the overlay is what makes an editor opened at that moment safe. The test is `cancels an armed cascade when the click lands on the grid behind it` in [odometer-cascade.spec.ts](./tests/integration/specs/tier2/odometer-cascade.spec.ts).

What that term does catch is the window before the modal. `TripGrid.handleDelete` awaits its dry run before it can set `pendingCascade`, and for the length of that round trip there is no modal, no overlay, and every control rule 2 names is live. An editor opened inside it holds a pre-cascade odometer that rule 1 will not re-seed, so confirming the delete moves the book underneath it -- C1 with the steps reversed, reached through the one arm path where no row is already open. `cascadeArming` ([TripGrid.svelte](./src/lib/components/TripGrid.svelte)) closes it: `handleDelete` raises the flag on entry and drops it in a `finally`, and the flag feeds the same `cascadePending` prop, so the editor, copy, insert above, delete and new record are all blocked for the window, not just the editor. The tooltips stay tied to the editing condition alone, because "Finish the open record first" is untrue while the flag is up and no record is open. The edit and insert arms need no flag of their own: a row is already open in both, so `otherRowEditing` and `newRowOpen` cover their windows. The test is `does not open a row editor while a cascade dry run is in flight`, which widens the window by holding the dry-run response for three seconds; both halves of it fail without the flag.

C1, found in the review of this branch and fixed before release. Rule 1 was in place. Rule 2 covered the copy button only. The sequence: row A is open in its editor and its form holds odometer `O_A`. The user deletes an earlier row of span `S` and confirms that delete's own modal. The book now stores `O_A - S` for row A, and row A's anchor is `S` lower too. The grid refreshes, but rule 1 blocks the re-seed while A is editing, so the form still holds `O_A`. The user then saves A. The km is unchanged and the odometer differs by `S`, so `plan_odometer_cascade` takes its odometer-wins branch and writes `distance_km := O_A - (anchor - S)`, which is A's real distance plus `S`. Nothing catches this. The value grows, so the negative-distance guard above cannot fire. The odometer-wins branch defines the row's span as its km, so the span check (ADR-043) cannot fire either. For the last row of the newest year no modal opens at all, because neither term of `needsApproval` ([TripGrid.svelte](./src/lib/components/TripGrid.svelte)) is true. An inflated `distance_km` lowers the computed l/100km, so the error hides a BIZ-003 breach rather than inventing one. An insert is the mirror image, with the sign reversed.

Each rule was correct on its own. Rule 1 protects what the user typed. Planning the apply from the stored book is what keeps a write true to the book. The seam is between them: the book moved while the editor held a copy of the old one, and nothing told the editor. The fix keeps both and completes rule 2, so the book cannot move under an open editor at all.

**Reasoning:** The distinction that separates this ADR from ADR-045 is what each kind of change does to a row's own relationship to its neighbour. A delta shift moves every affected row by the same amount, so an untouched row's span -- the gap between its own odometer and the anchor before it -- never changes. It creates no new span warning and clears none that already exists. A full rebase, by contrast, recomputes every odometer from the distances, so the chain agrees with itself by construction: every span becomes exactly its own distance, and the warnings that exist to find a real error (ADR-043) are erased along with it. That is why one is safe to run automatically on every save, and the other stays a manual, dry-run-first command (ADR-045).

**Related:** [Task 81](./_tasks/_done/81-odometer-cascade-on-save/); [ADR-045](#adr-045-the-odometer-rewrite-is-a-command-the-user-runs-never-a-side-effect-of-a-save) (the rebase this does not bring back); [ADR-043](#adr-043-the-span-is-the-check-that-finds-the-error-the-tied-datetime-only-explains-it) (the warning a rebase would erase); [Task 79](./_tasks/_done/79-odometer-span-inconsistency/) (a row below its anchor is on purpose); [BIZ-003](#biz-003-legal-margin-limit) (what a negative distance would corrupt); [ADR-008](#adr-008-remove-frontend-calculation-duplication) (the cascade has one home, in Rust; the two frontend flags are the guard that keeps it that way).
...
**Reasoning:** The order of the two middle keys was chosen by measurement, not by taste. Every figure in this section comes from [scripts/measure-trip-order.py](./scripts/measure-trip-order.py) -- the "Measured effect on the book" paragraph below is the one exception, and it names its own method -- run against a snapshot of the book taken on 2026-09-08 at 16:27, before rows 107 and 108 of 2026 were edited by hand: `_tmp/79-local/data/kniha-jazd.db.pre-run-162703`, md5 `ed75de8573ad2afceb5f0ed8a59c8594`, 329 trips, vehicle BT014IN. That path is gitignored, so the file does not travel with the repository; the script does, and it prints every number below from any copy of the book. Span warnings raised over the whole book, under the rule the app ships (`|odometer - odometer_start - distance_km| >= 1 km`):
...
The shipped order and odometer-first order disagreed in exactly one of the 31 tied groups, the 2026-08-19 pair -- those are the two orders the script compares, `(created_at, odometer, id)` against `(odometer, id)` ([measure-trip-order.py:114-121](./scripts/measure-trip-order.py)), not candidate 1 of the table above -- and there the place chain settles it: in the window "row before -> the tied group -> row after", `created_at` order connects 3 of 3 place links (Mlynske Nivy -> OMV, OMV -> Spisska Nova Ves, Spisska Nova Ves -> the next row), and odometer order connects 0 of 3.
...
**Superseded in part by [ADR-046](#adr-046-a-save-cascades-the-odometer-by-delta-a-rebase-never-runs-on-its-own):** the last sentence of the Decision below, "a save now writes exactly the row the user edited", is no longer true. A save now cascades a `delta` to every later row of the same year, behind a confirmation the user approves before anything is written. What this ADR still governs is the ban on the automatic full-year **rebase** below: `recalculate_odometers` stays a manual, UI-less, RPC-only command, and its 69-row-in-2023 / 68-row-in-2025 measurement still stands as the reason a rebase never runs on its own.
...
**Decision:** It does not. Nothing calls `recalculate_odometers` automatically, there is no UI for it, and it is reached over RPC only. It takes a `dryRun` flag: with `dryRun: true` it writes nothing and returns the rows it would change, each as `{tripId, tripNumber, oldOdometer, newOdometer}`. At the time of this decision, a save wrote exactly the row the user edited; [ADR-046](#adr-046-a-save-cascades-the-odometer-by-delta-a-rebase-never-runs-on-its-own) replaces that with a cascade the user approves first.

**Reasoning:** The measurement decided it. `recalculate_odometers` with `dryRun: true` reports what such a rewrite would do to the book: 0 rows in 2024, but **69 in 2023, 68 in 2025 and 3 in 2026**, none of them touched by the user. Those figures were measured on the local copy of 2026-09-08 that carries the 16:27 hand edit of two 2026 rows, not on production; the pre-edit snapshot answers 2 rows for 2026, and nobody ran the command against production. The 2023, 2024 and 2025 figures are the same on both copies. The 2023 run pushes the 22 km of row 1 through the whole year, which is the 223-row option the task 79 package argues against. The 2025 run moves 68 legal values by -0.5 km to chase a rounding artefact that the 1 km span tolerance ignores on purpose. Worse, both runs would erase the span warnings that exist to surface exactly those errors: a rewrite makes the chain agree with itself, so the sign disappears and the wrong data stays.
...
**Superseded (2026-09-11) by [ADR-050](#adr-050-paperless-ngx-is-the-only-invoice-source):** the `InvoiceRef` enum and the inline `InvoiceData` indirection are deleted. Paperless is the only source, so the commands take a concrete document id. The reasoning below is kept because it explains why the indirection existed.
...
**Superseded (2026-09-11) by [ADR-050](#adr-050-paperless-ngx-is-the-only-invoice-source):** no longer true. `paperless_trip_links` now carries a `mismatch_override` column, the assign command persists the flag, and `revert_paperless_override` clears it. The "deferred until a real user need surfaces" position below is closed.
...
**Superseded in part (2026-09-11) by [ADR-050](#adr-050-paperless-ngx-is-the-only-invoice-source):** the receipt half of the symmetry argument is gone with the `receipts` table. The link table itself stays, and its shape was already changed by the multi-invoice rebuild -- `trip_id` is no longer the primary key.
...
**Superseded (2026-09-11) by [ADR-050](#adr-050-paperless-ngx-is-the-only-invoice-source):** local receipts are deleted, so the year filter described below no longer has a table to filter. Paperless documents are filtered by year on the Paperless side.
