...
Rule 2 has a direction the UI does not enforce in `TripRow`, and does not need to. `handleEdit` runs only from a double-click on a display row, and the cascade modal's overlay is `position: fixed` over the whole viewport with a click-to-cancel handler ([OdometerCascadeModal.svelte](./src/lib/components/OdometerCascadeModal.svelte)). So the first click of any double-click aimed at the grid cancels the cascade, and `cascadePending` is already false by the time the second click reaches the row. Measured: the modal closes, the delete does not run, and the row opens on its stored odometer. The `cascadePending` term in `handleEdit` is therefore unreachable defence, kept but never exercised; the overlay is what makes an editor opened at that moment safe. The test is `cancels an armed cascade when the click lands on the grid behind it` in [odometer-cascade.spec.ts](./tests/integration/specs/tier2/odometer-cascade.spec.ts). One narrow hole remains open on purpose: `TripGrid.handleDelete` awaits its dry run before it sets `pendingCascade`, and during that await no overlay exists and no arm-time check reads `editingTripId`, so an editor opened inside that window survives into a confirmed cascade. The fix belongs at arm time in `TripGrid`, not in `TripRow.handleEdit`.
