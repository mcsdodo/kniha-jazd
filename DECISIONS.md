...
The frontend keeps two flags, `odoFollowsKm` and `manualOdoEdit` ([TripRow.svelte](./src/lib/components/TripRow.svelte)), but only as guards on `applyPreviewOdometer`. They decide nothing that reaches the backend. Without them, the live preview would overwrite the odometer of a row the user only opened -- a later save would then submit a changed odometer for a row nobody edited, and cascade the whole year on that basis. That is the bug commit `d101c08` already fixed once; keeping the two guards is what stops task 81 from reintroducing it.
