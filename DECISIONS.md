...
**Context:** [Task 81](./_tasks/81-odometer-cascade-on-save/) closes the gap ADR-045 left open. ADR-045 stopped a save from rewriting every later row of the year, but that also meant a save could leave the chain wrong right next to the row it touched: editing one row's distance left every later odometer stale, correctable only through the manual `recalculate_odometers` dry run. The fix is not to bring the old full-year rebase back. It is to move only what the edit itself changed, and to make the user approve that move first.
