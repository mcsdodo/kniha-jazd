...
/// Labels fixture shared by the export tests and the RPC dispatcher tests.
///
/// Only the strings the tests assert on carry real text - the rest are
/// placeholders so the fixture stays readable.
#[cfg(test)]
pub(crate) fn sample_export_labels() -> ExportLabels {
    ExportLabels {
        lang: "sk".to_string(),
