...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Initialize database
            let db =
                db::Database::new(db_paths.db_file.clone()).expect("Failed to initialize database");
            ...
            match db.check_migration_compatibility() {
                ...
                Err(unknown_migrations) => {
                    ...
                    app_state
                        .enable_read_only("Databáza bola aktualizovaná novšou verziou aplikácie.");
                }
            }
