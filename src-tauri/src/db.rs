...
impl Database {
    ...
    pub fn create_vehicle(&self, vehicle: &Vehicle) -> QueryResult<()> {
        ...
        let new_vehicle = NewVehicleRow {
            ...
            tank_size_liters: vehicle.tank_size_liters.map(|v| v as f32),
            tp_consumption: vehicle.tp_consumption.map(|v| v as f32),
            battery_capacity_kwh: vehicle.battery_capacity_kwh.map(|v| v as f32),
            baseline_consumption_kwh: vehicle.baseline_consumption_kwh.map(|v| v as f32),
            initial_battery_percent: vehicle.initial_battery_percent.map(|v| v as f32),
            initial_odometer: vehicle.initial_odometer as f32,
    ...
    pub fn update_vehicle(&self, vehicle: &Vehicle) -> QueryResult<()> {
        ...
        diesel::update(vehicles::table.filter(vehicles::id.eq(&id_str)))
            .set((
                ...
                vehicles::tank_size_liters.eq(vehicle.tank_size_liters.map(|v| v as f32)),
                vehicles::tp_consumption.eq(vehicle.tp_consumption.map(|v| v as f32)),
                vehicles::battery_capacity_kwh.eq(vehicle.battery_capacity_kwh.map(|v| v as f32)),
                vehicles::baseline_consumption_kwh
                    .eq(vehicle.baseline_consumption_kwh.map(|v| v as f32)),
                vehicles::initial_battery_percent
                    .eq(vehicle.initial_battery_percent.map(|v| v as f32)),
                vehicles::initial_odometer.eq(vehicle.initial_odometer as f32),
    ...
    pub fn create_trip(&self, trip: &Trip) -> QueryResult<()> {
        ...
        let new_trip = NewTripRow {
            ...
            distance_km: trip.distance_km as f32,
            odometer: trip.odometer as f32,
            purpose: &trip.purpose,
            fuel_liters: trip.fuel_liters.map(|v| v as f32),
            fuel_cost_eur: trip.fuel_cost_eur.map(|v| v as f32),
            other_costs_eur: trip.other_costs_eur.map(|v| v as f32),
            other_costs_note: other_costs_note_ref,
            full_tank: if trip.full_tank { 1 } else { 0 },
            sort_order: trip.sort_order,
            energy_kwh: trip.energy_kwh.map(|v| v as f32),
            energy_cost_eur: trip.energy_cost_eur.map(|v| v as f32),
            full_charge: Some(if trip.full_charge { 1 } else { 0 }),
            soc_override_percent: trip.soc_override_percent.map(|v| v as f32),
            created_at: &created_at_str,
            updated_at: &updated_at_str,
        };
    ...
    pub fn update_trip(&self, trip: &Trip) -> QueryResult<()> {
        ...
        diesel::update(trips::table.filter(trips::id.eq(&id_str)))
            .set((
                ...
                trips::distance_km.eq(trip.distance_km as f32),
                trips::odometer.eq(trip.odometer as f32),
                trips::purpose.eq(&trip.purpose),
                trips::fuel_liters.eq(trip.fuel_liters.map(|v| v as f32)),
                trips::fuel_cost_eur.eq(trip.fuel_cost_eur.map(|v| v as f32)),
                trips::other_costs_eur.eq(trip.other_costs_eur.map(|v| v as f32)),
                ...
                trips::energy_kwh.eq(trip.energy_kwh.map(|v| v as f32)),
                trips::energy_cost_eur.eq(trip.energy_cost_eur.map(|v| v as f32)),
                trips::full_charge.eq(Some(if trip.full_charge { 1 } else { 0 })),
                trips::soc_override_percent.eq(trip.soc_override_percent.map(|v| v as f32)),
    ...
    pub fn create_route(&self, route: &Route) -> QueryResult<()> {
        ...
        let new_route = NewRouteRow {
            ...
            distance_km: route.distance_km as f32,
    ...
    pub fn update_route(&self, route: &Route) -> QueryResult<()> {
        ...
        diesel::update(routes::table.filter(routes::id.eq(&id_str)))
            .set((
                ...
                routes::distance_km.eq(route.distance_km as f32),
    ...
    pub fn create_receipt(&self, receipt: &Receipt) -> QueryResult<()> {
        ...
        let new_receipt = NewReceiptRow {
            ...
            liters: receipt.liters.map(|v| v as f32),
            total_price_eur: receipt.total_price_eur.map(|v| v as f32),
    ...
    pub fn update_receipt(&self, receipt: &Receipt) -> QueryResult<()> {
        ...
        diesel::update(receipts::table.filter(receipts::id.eq(&id_str)))
            .set((
                ...
                receipts::liters.eq(receipt.liters.map(|v| v as f32)),
                receipts::total_price_eur.eq(receipt.total_price_eur.map(|v| v as f32)),
