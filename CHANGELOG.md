...
### Zmenené
- **Uloženie záznamu už neprepisuje tachometer ďalších riadkov** - doteraz aplikácia po každom uložení prepočítala stav tachometra vo všetkých nasledujúcich záznamoch daného roka, aj tie, ktorých si sa nedotkol. Teraz sa uloží len ten riadok, ktorý si upravil. Hromadný prepočet zostal ako samostatný príkaz s náhľadom - najprv vypíše, ktoré riadky a na aké hodnoty by zmenil, a nezapíše nič. Spúšťa sa cez rozhranie RPC a v aplikácii preň nie je tlačidlo. Automatický prepočet mal totiž aj druhú cenu: prepísaný reťazec sedí sám so sebou, takže by zmazal aj upozornenia na nesúlad tachometra - a práve tie chybu ukazujú.
